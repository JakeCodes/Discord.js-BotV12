...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0])
      return message.channel.send(
        `Please specify a song to play ${emotes.error}`
      );
    ...
    if (aTrackIsAlreadyPlaying) {
      ...
      if (result.type === "playlist") {
        message.channel.send(
          `${result.tracks.length} songs added to the queue ${emotes.music}`
        );
      } else {
        message.channel.send(
          `${result.name} added to the queue ${emotes.music}`
        );
      }
    } else {
      //Else, play the song
      ...
      if (result.type === "playlist") {
        ...
        message.channel.send(
          `Currently playing ${result.name} ${emotes.music}`
        );
      }

      const queue = client.player
